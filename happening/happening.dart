...
/**
 * An [Observer] function is used to observe a [Happening] and will be invoked each time
 * the [Happening] happens until it ignores the [Happening].  The [Happening] may pass a different
 * [context] to the [Observer] each time it happens, and the [context] may be [:null:].
 */
 ...
 *    }
 *    
 * Our Kitten can meow and it can eat any given Mouse.  These 2 occurences are exposed via the
 * Happenings meowed and ate.  When the Kitten eats a Mouse, it also passes the specific Mouse
 * that it ate as the context of that Happening.  Now let's define an Owner class as someone
 * who wants to observe the behavior of one or more Kittens:
 * 
 *    class Owner {
 ...
 *    }
 *    
 *  The first thing to notice is that the Owner keeps a private Observer reference to each
 *  of its observer methods.  These are the references that it will use to observe and ignore
 *  the Happenings.  When the Owner adopts a new Kitten, we can see that it starts to observe
 *  both the meowed and ate happenings using these references.  Those calls are wrapped in 
 *  assert statements as a convenience, to ensure that the Owner does not accidentally observe 
 *  the same Happening twice.  When the owner releases a Kitten back to the wild, it similarly 
 *  ignores the Happenings.  This example also illustrates the use case of observing multiple 
 *  Happenings with the same Observer reference.  Since the Owner controls the Observer reference 
...
class Happening<T> {
  ...
  /**
   * Notifies all observers that the happening has happened.  A [context] may be provided
   * and will be passed to all observers.
   */
  void happen([T context]) => _observers.forEach((o) => o(context));
  
  /**
   * Ignores the happening for the given [observer].  Returns [:true:] if the [observer]
   * was observing the happening, else [:false:].
   */
  ...
  /**
   * Observes the happening for the given [observer].  Returns [:true:] if the [observer]
   * was not already observing the happening, else [:false:].
   */
